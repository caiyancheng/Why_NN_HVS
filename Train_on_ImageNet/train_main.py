#!/usr/bin/env python3
"""
Fast ImageNet trainer for 2x RTX 4090 (PyTorch DDP + AMP)
"""
import argparse
import math
import os
...
def main():
    p = argparse.ArgumentParser(description="Fast ImageNet Trainer (2x4090)")
    ...
    train_sampler = DistributedSampler(train_set, num_replicas=world_size, rank=dist.get_rank()) if dist.is_initialized() else None
    val_sampler   = DistributedSampler(val_set,   num_replicas=world_size, rank=dist.get_rank(), shuffle=False) if dist.is_initialized() else None
    train_loader = DataLoader(train_set, batch_size=args.batch_size, sampler=train_sampler, shuffle=(train_sampler is None), num_workers=args.workers, pin_memory=True, persistent_workers=True, prefetch_factor=4)
    val_loader = DataLoader(val_set, batch_size=args.batch_size, sampler=val_sampler, shuffle=False, num_workers=args.workers, pin_memory=True, persistent_workers=True, prefetch_factor=4)
    ...
    if args.resume and os.path.isfile(args.resume):
        ...
        if 'model' in checkpoint:
            model.module.load_state_dict(checkpoint['model']) if isinstance(model, DDP) else model.load_state_dict(checkpoint['model'])
    ...
    for epoch in tqdm(range(start_epoch, args.epochs)):
        ...
        tr_loss, tr_top1, tr_time = train_one_epoch(model, train_loader, optimizer, scaler, device, epoch, loss_fn, scheduler, channels_last=args.channels_last)
        val_loss, val_top1, val_top5 = validate(model, val_loader, device, loss_fn)
        if is_main_process():
            print(f"Epoch {epoch+1}/{args.epochs} | train loss {tr_loss:.3f} top1 {tr_top1:.2f} | val loss {val_loss:.3f} top1 {val_top1:.2f} top5 {val_top5:.2f} | epoch time {tr_time/60:.1f} min")
            payload = {
                'epoch': epoch + 1,
